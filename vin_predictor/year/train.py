...
class VINYearPredictor:
    ...
    def load_and_preprocess_data(self, csv_file_path):
        ...
        df['brand'] = df['brand'].apply(unify_brand_name)  # Unify brand names
        ...
        df = df[df.duplicated(subset=['brand','year'], keep=False)] # keep only duplicated brand-year pairs
    ...
    def prepare_labels(self, brand_years):
        """
        Encode brand labels as integers
        """
        if self.binarizer is None:
            self.binarizer = MultiLabelBinarizer()
            binarized_labels = self.binarizer.fit_transform(brand_years)
        else:
            binarized_labels = self.binarizer.transform(brand_years)
    ...
    def train_model(self, csv_file_path, test_size=0.2, epochs=50, batch_size=32):
        ...
        year_tuples = [tuple(row) for row in df[[ 'year']].values]
        y = self.prepare_labels(year_tuples)
        ...
        # Build model
        num_classes = len(self.binarizer.classes_)
        print(f"Number of classes: {num_classes}")
        print(f"Classes: {self.binarizer.classes_}")
    ...
    def plot_confusion_matrix(self):
        ...
        sns.heatmap(cm, 
                   annot=True, 
                   fmt='d', 
                   xticklabels=self.binarizer.classes_,
                   yticklabels=self.binarizer.classes_)
    ...
    def predict(self, vins):
        ...
        predicted_labels = self.binarizer.inverse_transform(predicted_binary)
    ...
    def save_model(self, model_path="vin_year_model"):
        ...
        with open(f"{model_path}_binarizer.pkl", 'wb') as f:
    ...
    def load_model(self, model_path="vin_year_model"):
        ...
        with open(f"{model_path}_binarizer.pkl", 'rb') as f:
...
if __name__ == "__main__":
    ...
    # Save the model
    predictor.save_model("models/year/vin_year_predictor")